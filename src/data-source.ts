...
export const AppDataSource = new DataSource({
  ...
  database: process.env.DB_NAME || "shipping-service",
...
async function checkPostgreSQLExtensions() {
  ...
  const shipping = await pool.connect();

  try {
    for (const ext of REQUIRED_EXTENSIONS) {
      const res = await shipping.query(
      ...
      if (res.rows.length === 0) {
        ...
        await shipping.query('CREATE EXTENSION IF NOT EXISTS "' + ext + '"');
      }
    }
  } finally {
    await shipping.release();
    await pool.end();
  }
}
