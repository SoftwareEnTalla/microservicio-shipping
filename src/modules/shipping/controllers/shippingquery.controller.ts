...
@ApiTags("Shipping Query")
@UseGuards(ShippingAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("shippings/query")
export class ShippingQueryController {
  #logger = new Logger(ShippingQueryController.name);

  constructor(private readonly service: ShippingQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all shipping with optional pagination" })
  @ApiResponse({ status: 200, type: ShippingsResponse })
  @ApiQuery({ name: "options", required: false, type: ShippingDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<Shipping>    
  ): Promise<ShippingsResponse<Shipping>> {
    try {
     
      const shippings = await this.service.findAll(options);
      logger.info("Retrieving all shipping");
      return shippings;
    } catch (error) {
      logger.error(error);
  ...
  @ApiOperation({ summary: "Find shipping by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter shipping', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: ShippingsResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Shipping no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find shippings with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<Shipping>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all shippings" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count shippings with conditions" })
  @ApiResponse({ status: 200, type: ShippingsResponse<Shipping> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one shipping with conditions" })
  ...
  @ApiOperation({ summary: "Find one shipping or return error" })
  @ApiResponse({ status: 200, type: ShippingResponse<Shipping> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @ApiParam({ name: 'id', required: true, description: 'ID of the shipping to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<ShippingResponse<Shipping>> {
    try {
      const shipping = await this.service.findOne({ where: { id } });
      if (!shipping) {
        throw new NotFoundException(
